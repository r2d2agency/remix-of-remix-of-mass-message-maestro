...
-- Sync history for Google Calendar status panel
...
DO $$ BEGIN
    ...
    ALTER TABLE group_secretary_config ADD COLUMN IF NOT EXISTS task_board_column_id UUID DEFAULT NULL;
EXCEPTION WHEN duplicate_column THEN null; END $$;
...
const step44Meetings = `
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'meeting_type') THEN
    CREATE TYPE meeting_type AS ENUM (
      'atendimento_inicial','reuniao_cliente','audiencia_remota',
      'reuniao_estrategica','reuniao_interna','alinhamento_processual','outro'
    );
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'meeting_status') THEN
    CREATE TYPE meeting_status AS ENUM (
      'aguardando_transcricao','transcrevendo','resumo_gerado',
      'pendente_revisao','finalizado','com_pendencias'
    );
  END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_meetings_lawyer ON meetings(lawyer_user_id);
...
const step46DocumentsSignatures = `
...
CREATE TABLE IF NOT EXISTS signature_requests (
    ...
    send_channel VARCHAR(50) DEFAULT 'whatsapp',
...
const migrationSteps = [
  ...
  { name: 'Documents & Signatures', sql: step46DocumentsSignatures, critical: false },
];
