...
function extractUazapiMessage(payload) {
  ...
  return {
    ...
    chatId: pickFirstString(data.chatid, data.chatId, data.remoteJid, data.from, data.to, payload?.chatid, payload?.chatId, payload?.chat?.wa_chatid, payload?.message?.chatid),
    sender: pickFirstString(data.sender, data.sender_pn, data.sender_lid, data.owner, payload?.message?.sender),
    senderName: pickFirstString(data.senderName, data.pushName, data.name, payload?.message?.senderName, payload?.chat?.name),
    fromMe: data.fromMe === true || data.wasSentByApi === true || payload?.message?.fromMe === true,
    isGroup: data.isGroup === true || payload?.chat?.wa_isGroup === true || payload?.message?.isGroup === true || String(data.chatid || data.chatId || payload?.chat?.wa_chatid || payload?.message?.chatid || '').includes('@g.us'),
    messageType,
