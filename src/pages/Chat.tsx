  ...
  return (
    <MainLayout>
      ...
      <div className={cn(
        ...
        isMobile 
          ? "h-[100dvh] w-full fixed inset-0 z-30 rounded-none border-0 pt-[env(safe-area-inset-top,0px)]" 
          : "h-[calc(100dvh-5rem)] lg:h-[calc(100dvh-5.5rem)] xl:h-[calc(100dvh-6rem)]"
      )}>
        ...
        {isMobile ? (
          <div className="flex-1 flex overflow-hidden min-w-0 w-full min-h-0">
          ...
          >
            <ResizablePanel defaultSize={(() => { try { const s = JSON.parse(localStorage.getItem('chat-panel-sizes') || ''); return s[0] ?? 25; } catch { return 25; } })()} minSize={15} maxSize={45} className="!flex !flex-col h-full min-h-0 overflow-hidden min-w-0">
              <ConversationList
                conversations={conversations}
                selectedId={selectedConversation?.id || null}
                onSelect={handleMobileSelectConversation}
            ...
            {selectedConversation && modulesEnabled.crm && crmPanelOpen && (
              <>
                ...
                <ResizablePanel 
                  ...
                  minSize={15} 
                  maxSize={40} 
                  className="relative !flex !flex-col min-h-0 h-full overflow-hidden"
                >
                  <CRMSidePanel conversationId={selectedConversation.id}
                    contactPhone={selectedConversation.remote_jid?.replace('@s.whatsapp.net', '').replace('@g.us', '') || null}
                    contactName={selectedConversation.contact_name || selectedConversation.group_name || null}
